...
function validateConfig() {
  if (!config.accounts || config.accounts.length === 0) {
    log('ERROR: No accounts configured in config.json', 'ERROR');
    console.log('\nPlease update your config.json file with your accounts:');
  ...
  for (let i = 0; i < config.accounts.length; i++) {
    ...
    if (!account.username || !account.password) {
      log(`ERROR: Username and password must be set for account #${i+1} in config.json`, 'ERROR');
      return false;
    }
  }
  return true;
...
class TokenManager {
  constructor(username, password, accountIndex) {
    ...
    this.tokenPath = path.join(__dirname, `tokens_${accountIndex}.json`);
  }
  ...
  async refreshOrAuthenticate() {
    try {
      ...
      log(`Token refresh/auth error for account ${this.accountIndex}: ${error.message}`, 'ERROR');
      throw error;
    }
  }
  ...
  async updateTokens(result) {
    ...
    log(`Tokens updated and saved for account ${this.accountIndex}`);
  }
  ...
  async getTokens() {
    try {
      ...
      log(`Successfully read access token for account ${this.accountIndex}: ${tokens.accessToken.substring(0, 10)}...`);
      return tokens;
    } catch (error) {
      log(`Error reading tokens for account ${this.accountIndex}: ${error.message}`, 'ERROR');
      throw error;
    }
  }
  ...
  async saveTokens(tokens) {
    try {
      ...
      log(`Tokens saved successfully for account ${this.accountIndex}`);
      return true;
    } catch (error) {
      log(`Error saving tokens for account ${this.accountIndex}: ${error.message}`, 'ERROR');
      return false;
    }
  }
}
...
async function refreshTokens(refreshToken, accountIndex) {
  try {
    log(`Refreshing access token via Stork API for account ${accountIndex}...`);
    ...
    const tokenPath = path.join(__dirname, `tokens_${accountIndex}.json`);
    await fs.promises.writeFile(tokenPath, JSON.stringify(tokens, null, 2), 'utf8');
    log(`Token refreshed successfully via Stork API for account ${accountIndex}`);
    return tokens;
  } catch (error) {
    log(`Token refresh failed for account ${accountIndex}: ${error.message}`, 'ERROR');
    throw error;
  }
}
...
async function getUserStats(tokens, accountIndex) {
  try {
    log(`Fetching user stats for account ${accountIndex}...`);
    const response = await axios({
      method: 'GET',
    ...
    log(`Error getting user stats for account ${accountIndex}: ${error.message}`, 'ERROR');
    throw error;
  }
}
...
if (!isMainThread) {
  const { priceData, tokens, proxy, accountIndex } = workerData;

  async function validateAndSend() {
    try {
      const isValid = validatePrice(priceData);
      await sendValidation(tokens, priceData.msg_hash, isValid, proxy);
      parentPort.postMessage({ success: true, msgHash: priceData.msg_hash, isValid, accountIndex });
    } catch (error) {
      parentPort.postMessage({ success: false, error: error.message, msgHash: priceData.msg_hash, accountIndex });
    }
  }

  validateAndSend();
} else {
  // Store previous stats for each account
  const accountStats = {};

  async function runValidationProcess(tokenManager, accountIndex) {
    try {
      log(`--------- STARTING VALIDATION PROCESS FOR ACCOUNT ${accountIndex} ---------`);
      const tokens = await tokenManager.getTokens();
      const initialUserData = await getUserStats(tokens, accountIndex);

      if (!initialUserData || !initialUserData.stats) {
        throw new Error(`Could not fetch initial user stats for account ${accountIndex}`);
      }
      ...
      if (!accountStats[accountIndex]) {
        accountStats[accountIndex] = { validCount: initialValidCount, invalidCount: initialInvalidCount };
      }
      ...
      if (!signedPrices || signedPrices.length === 0) {
        log(`No data to validate for account ${accountIndex}`);
        const userData = await getUserStats(tokens, accountIndex);
        displayStats(userData, accountIndex);
        return;
      }

      log(`Processing ${signedPrices.length} data points with ${config.threads.maxWorkers} workers for account ${accountIndex}...`);
      const workers = [];
        ...
        batch.forEach(priceData => {
          workers.push(new Promise((resolve) => {
            const worker = new Worker(__filename, {
              workerData: { priceData, tokens, proxy, accountIndex }
            });
            worker.on('message', resolve);
            worker.on('error', (error) => resolve({ success: false, error: error.message, accountIndex }));
            worker.on('exit', () => resolve({ success: false, error: 'Worker exited', accountIndex }));
          }));
        });
      }
      ...
      log(`Processed ${successCount}/${results.length} validations successfully for account ${accountIndex}`);

      const updatedUserData = await getUserStats(tokens, accountIndex);
      ...
      const actualValidIncrease = newValidCount - accountStats[accountIndex].validCount;
      const actualInvalidIncrease = newInvalidCount - accountStats[accountIndex].invalidCount;

      accountStats[accountIndex].validCount = newValidCount;
      accountStats[accountIndex].invalidCount = newInvalidCount;

      displayStats(updatedUserData, accountIndex);
      log(`--------- VALIDATION SUMMARY FOR ACCOUNT ${accountIndex} ---------`);
      ...
      log(`--------- COMPLETE FOR ACCOUNT ${accountIndex} ---------`);
    } catch (error) {
      log(`Validation process stopped for account ${accountIndex}: ${error.message}`, 'ERROR');
    }
  }

  function displayStats(userData, accountIndex) {
    if (!userData || !userData.stats) {
      log(`No valid stats data available to display for account ${accountIndex}`, 'WARN');
      return;
    }

    console.log('=============================================');
    console.log(`   STORK ORACLE AUTO BOT - ACCOUNT ${accountIndex}   `);
    console.log('=============================================');
  ...
  function displayAllStats() {
    console.clear();
    console.log('=============================================');
    console.log('   STORK ORACLE MULTI-ACCOUNT AUTO BOT   ');
    console.log('=============================================');
    console.log(`Time: ${getTimestamp()}`);
    console.log(`Running ${config.accounts.length} accounts`);
    console.log('=============================================');
    console.log('   Created by 568   ');
    console.log('=============================================');
  }
  ...
  async function main() {
    ...
    // Initialize token managers for each account
    for (let i = 0; i < config.accounts.length; i++) {
      const account = config.accounts[i];
      tokenManagers.push(new TokenManager(account.username, account.password, i));
    }

    try {
      // Initial authentication for all accounts
      for (let i = 0; i < tokenManagers.length; i++) {
        await tokenManagers[i].getValidToken();
        log(`Initial authentication successful for account ${i}`);
      }

      // Display header with all accounts
      displayAllStats();

      // Run validation process for each account
      for (let i = 0; i < tokenManagers.length; i++) {
        runValidationProcess(tokenManagers[i], i);
      }

      // Set interval to run validation for all accounts
      setInterval(() => {
        displayAllStats();
        for (let i = 0; i < tokenManagers.length; i++) {
          runValidationProcess(tokenManagers[i], i);
      ...
      // Set interval to refresh tokens for all accounts
      setInterval(async () => {
        for (let i = 0; i < tokenManagers.length; i++) {
          await tokenManagers[i].getValidToken();
          log(`Token refreshed via Cognito for account ${i}`);
        }
      }, 50 * 60 * 1000);
    } catch (error) {
